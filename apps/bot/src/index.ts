...
if (!appConfig.telegramBotEnabled) {
  console.log(
    "PulsorClip Telegram bot disabled by TELEGRAM_BOT_ENABLED=false.",
  );
  process.exit(0);
}
  ...
  try {
    ...
    if (appConfig.telegramAdminIds.length === 0) {
      logServer("warn", "bot.bootstrap.admins.none", {
        message:
          "⚠️ NO TELEGRAM_ADMIN_IDS configured. Admin notifications (start, daily, health) will be SKIPPED.",
    ...
    const reachableAdmins = adminValidation
      .filter((item) => item.ok)
    ...
    if (appConfig.telegramAdminIds.length > 0 && reachableAdmins.length === 0) {
      logServer("error", "bot.bootstrap.admins.unreachable", {
        message:
          "❌ ALL configured TELEGRAM_ADMIN_IDS are unreachable! You will NOT receive notifications.",
    ...
    const startupResult = await notifyAdmins(
      bot,
      `${adminMessage}\n${appConfig.baseUrl}`,
    );
    ...
    if (reachableAdmins.length > 0 && startupResult.delivered === 0) {
      logServer("error", "bot.bootstrap.admin_notify.failed_all", {
        message:
          "❌ Startup notification failed to reach ANY admins, even though they were marked as reachable.",
